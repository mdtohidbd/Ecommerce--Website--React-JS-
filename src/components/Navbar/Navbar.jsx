...
const Navbar = () => {
  ...
  const toggleMenu = () => {
    setShowMenu(!showMenu)   //it will toggle if the showmenu is false it will be true and if true it will be
  }
  return (
    <div className='px-4 fixed w-full z-40 shadow-lg top-0 shadow-gray-500 bg-red-100'>
        ...
        <div className='flex items-center gap-5'>
            <nav className='hidden md:block'>
                <ul className='flex items-center font-semibold text-xl gap-7'>
                    ...
                    <Link to='/login'><button className='bg-red-400 text-white px-4 py-1 rounded-md shadow-sm'>Login</button></Link>
                    
                </ul>
            </nav>
            <Link to='/cart' className='relative w-12'>
            <ShoppingCart/> 
